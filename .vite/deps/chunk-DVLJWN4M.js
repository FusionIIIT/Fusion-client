  ...
  const paginationRange = (0, import_react26.useMemo)(() => {
    ...
    if (shouldShowLeftDots && !shouldShowRightDots) {
      ...
      return [
        ...range2(1, boundaries),
        DOTS,
        ...range2(_total - rightItemCount, _total),
      ];
    }
    return [
      ...range2(1, boundaries),
        ...
        ? new ResizeObserver((entries) => {
            ...
            if (entry) {
              ...
              frameID.current = requestAnimationFrame(() => {
                ...
                if (ref.current) {
                  const boxSize =
                    ((_a = entry.borderBoxSize) == null ? void 0 : _a[0]) ||
                    ((_b = entry.contentBoxSize) == null ? void 0 : _b[0]);
                  if (boxSize) {
...
function useThrottledState(defaultValue, wait) {
  ...
  const [setThrottledValue, clearTimeout2] =
...
function useThrottledValue(value, wait) {
  const [throttledValue, setThrottledValue] = (0, import_react63.useState)(
    value,
  );
  const valueRef = (0, import_react63.useRef)(value);
  const [throttledSetValue, clearTimeout2] =
...
function getInitialValue2(initialValue, getInitialValueInEffect) {
  ...
  if (typeof window !== "undefined" && "screen" in window) {
    return {
      angle:
        ((_a = window.screen.orientation) == null ? void 0 : _a.angle) ??
        initialValue.angle,
      type:
        ((_b = window.screen.orientation) == null ? void 0 : _b.type) ??
        initialValue.type,
    };
  }
  return initialValue;
...
function useOrientation({
  ...
  useIsomorphicEffect(() => {
    var _a;
    (_a = window.screen.orientation) == null
      ? void 0
      : _a.addEventListener("change", handleOrientationChange);
    return () => {
      var _a2;
      return (_a2 = window.screen.orientation) == null
        ? void 0
        : _a2.removeEventListener("change", handleOrientationChange);
    };
  }, []);
  return orientation;
}
...
function getDigitsAfterDot(value) {
  var _a;
  return (
    ((_a = value.toString().split(".")[1]) == null ? void 0 : _a.length) || 0
  );
}
...
function useRadialMove(
  onChange,
  { step = 0.01, onChangeEnd, onScrubStart, onScrubEnd } = {},
) {
...
function getHeadingsData(headings, getDepth, getValue) {
  ...
  for (let i = 0; i < headings.length; i += 1) {
    ...
    result.push({
      ...
      getNode: () =>
        heading.id ? document.getElementById(heading.id) : heading,
    });
  }
  return result;
}
...
function useFileDialog(input = {}) {
  ...
  const [files, setFiles] = (0, import_react69.useState)(
...
//# sourceMappingURL=chunk-DVLJWN4M.js.map
