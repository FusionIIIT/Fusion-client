import { require_jsx_runtime } from "./chunk-RZSASJON.js";
...
function createFormActions(name) {
  ...
  const setFieldError = (path, error) =>
  ...
  const reorderListItem = (path, payload) =>
  ...
  const insertListItem = (path, item, index) =>
...
function useFormActions(name, form) {
  ...
  useFormEvent(`mantine-form:${name}:set-field-value`, (event) =>
    form.setFieldValue(event.detail.path, event.detail.value),
  );
  useFormEvent(`mantine-form:${name}:set-values`, (event) =>
    form.setValues(event.detail),
  );
  useFormEvent(`mantine-form:${name}:set-initial-values`, (event) =>
    form.setInitialValues(event.detail),
  );
  useFormEvent(`mantine-form:${name}:set-errors`, (event) =>
    form.setErrors(event.detail),
  );
  useFormEvent(`mantine-form:${name}:set-field-error`, (event) =>
    form.setFieldError(event.detail.path, event.detail.error),
  );
  useFormEvent(`mantine-form:${name}:clear-field-error`, (event) =>
  ...
  useFormEvent(`mantine-form:${name}:validate-field`, (event) =>
    form.validateField(event.detail),
  );
  useFormEvent(`mantine-form:${name}:reorder-list-item`, (event) =>
    form.reorderListItem(event.detail.path, event.detail.payload),
  );
  useFormEvent(`mantine-form:${name}:remove-list-item`, (event) =>
    form.removeListItem(event.detail.path, event.detail.index),
  );
  useFormEvent(`mantine-form:${name}:insert-list-item`, (event) =>
    form.insertListItem(
      event.detail.path,
      event.detail.item,
      event.detail.index,
  ...
  useFormEvent(`mantine-form:${name}:set-dirty`, (event) =>
    form.setDirty(event.detail),
  );
  useFormEvent(`mantine-form:${name}:set-touched`, (event) =>
    form.setTouched(event.detail),
  );
  useFormEvent(`mantine-form:${name}:reset-dirty`, (event) =>
...
function useFormErrors(initialErrors) {
  const [errorsState, setErrorsState] = (0, import_react2.useState)(
    filterErrors(initialErrors),
  );
...
function useFormList({ $values, $errors, $status }) {
  ...
  const replaceListItem = (0, import_react3.useCallback)(
    (path, index, item) => {
      $status.clearFieldDirty(path);
      $values.setValues({
...
function getStatus(status, path) {
  ...
  if (typeof path === "string") {
    const nestedPaths = paths.filter((statusPath) =>
      statusPath.startsWith(`${path}.`),
    );
    return (
      status[path] ||
      nestedPaths.some((statusPath) => status[statusPath]) ||
      false
    );
  }
...
function useFormStatus({ initialDirty, initialTouched, mode, $values }) {
  const [touchedState, setTouchedState] = (0, import_react4.useState)(
    initialTouched,
  );
  ...
  const setTouched = (0, import_react4.useCallback)((values) => {
    const resolvedValues =
      typeof values === "function" ? values(touchedRef.current) : values;
  ...
  const setDirty = (0, import_react4.useCallback)(
    (values, forceUpdate = false) => {
      const resolvedValues =
        typeof values === "function" ? values(dirtyRef.current) : values;
  ...
  const setFieldDirty = (0, import_react4.useCallback)(
    (path, dirty, forceUpdate) => {
  ...
  const setCalculatedFieldDirty = (0, import_react4.useCallback)(
    (path, value) => {
      const currentDirty = getStatus(dirtyRef.current, path);
      const dirty = !(0, import_fast_deep_equal.default)(
        getPath(path, $values.getValuesSnapshot()),
        value,
      );
  ...
  const isDirty = (0, import_react4.useCallback)((path) => {
    if (path) {
      ...
      const sliceOfInitialValues = getPath(
        path,
        $values.valuesSnapshot.current,
      );
      return !(0, import_fast_deep_equal.default)(
        sliceOfValues,
        sliceOfInitialValues,
      );
    }
  ...
  const getTouched = (0, import_react4.useCallback)(
    () => touchedRef.current,
    [],
  );
  return {
...
function useFormValues({ initialValues, onValuesChange, mode }) {
  ...
  const [stateValues, setStateValues] = (0, import_react5.useState)(
    initialValues || {},
  );
  ...
  const setValues = (0, import_react5.useCallback)(
    ({
      ...
      const resolvedValues =
        values instanceof Function ? values(refValues.current) : values;
      ...
      subscribers == null
        ...
        : subscribers
            .filter(Boolean)
            .forEach((subscriber) =>
              subscriber({ updatedValues, previousValues }),
            );
    },
    [onValuesChange],
  );
  ...
  const getValuesSnapshot = (0, import_react5.useCallback)(
    () => valuesSnapshot.current,
    [],
  );
  return {
    initialized,
...
function shouldValidateOnChange(path, validateInputOnChange) {
  ...
  if (Array.isArray(validateInputOnChange)) {
    return validateInputOnChange.includes(
      path.replace(/[.][0-9]+/g, `.${FORM_INDEX}`),
    );
  }
  return false;
}
...
function useForm({
      ...
      Object.keys($watch.subscribers.current).forEach((path) => {
        ...
        if (value !== previousValue) {
          $watch
            ...
            .forEach((cb) =>
              cb({ previousValues, updatedValues: $values.refValues.current }),
            );
        }
      });
    },
      ...
      $values.setFieldValue({
        ...
        subscribers: [
          ...
          shouldValidate
            ...
            : null,
          (options == null ? void 0 : options.forceUpdate) !== false &&
          mode !== "controlled"
            ? () =>
                setFieldKeys((keys) => ({
                  ...keys,
  ...
  const getInputProps = (
    ...
    const onChange = getInputOnChange((value) =>
      setFieldValue(path, value, { forceUpdate: false }),
    );
  ...
  const getTransformedValues = (input) =>
    transformValues(input || $values.refValues.current);
...
var Form = (0, import_react9.forwardRef)(
  ({ form, onSubmit, onReset, ...others }, ref) =>
...
function useField({
  ...
  const [valueState, setValueState] = (0, import_react10.useState)(
    initialValue,
  );
  ...
  const setTouched = (0, import_react10.useCallback)(
    (val, { updateState = mode === "controlled" } = {}) => {
  ...
  const isTouched = (0, import_react10.useCallback)(
    () => touchedRef.current,
    [],
  );
  const isDirty = (0, import_react10.useCallback)(
    () => valueRef.current !== initialValue,
    [initialValue],
  );
  const _validate = (0, import_react10.useCallback)(async () => {
    const validationResult =
      validate == null ? void 0 : validate(valueRef.current);
  ...
  const getInputProps = ({ withError = true, withFocus = true } = {}) => {
    const onChange = getInputOnChange((val) =>
      setValue(val, { updateKey: false }),
    );
    ...
    if (type === "checkbox") {
      ...
      payload[mode === "controlled" ? "value" : "defaultValue"] =
        valueRef.current;
    }
    if (withFocus) {
      payload.onFocus = () => {
  ...
  const resetTouched = (0, import_react10.useCallback)(
    () => setTouched(false),
    [],
  );
  return {
...
function superstructResolver(schema) {
  function structValidation(values) {
    ...
    err.failures().forEach((fieldFailure) => {
      ...
      formErrors[fieldFailure.path.join(".")] =
        `${fieldName}: ${fieldFailure.message}`;
    });
    return formErrors;
  }
