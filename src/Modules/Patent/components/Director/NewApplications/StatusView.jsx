...
function PatentApplication() {
  ...
  const handleReject = async () => {
    if (!feedback || feedback.trim().length < 50) {
      alert("Please provide feedback of at least 50 characters before rejecting.");
      return;
    }
    try {
      await axios.post(
        `${API_BASE_URL}/director/application/reject`,
        { application_id: applicationId, comments: feedback, decision: "Reject" },
        {
          headers: {
            Authorization: `Token ${authToken}`,
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    ...
    const fetchBudgetDetails = async () => {
      if (!applicationId) return;
      try {
        const res = await axios.get(`${API_BASE_URL}/director/budget/${applicationId}/decision/`, {
  ...
  const handleBudgetDecision = async (approve) => {
    try {
      setBudgetActionLoading(true);
      await axios.post(
        `${API_BASE_URL}/director/budget/${applicationId}/decision/`,
        { approve, remarks: feedback || "Director review" },
        { headers: { Authorization: `Token ${authToken}` } }
      );
      // Refresh budget
      const res = await axios.get(`${API_BASE_URL}/director/budget/${applicationId}/decision/`, {
      ...
      if (res.data) setBudgetData(res.data);
      alert(`Budget ${approve ? "Approved" : "Denied"} successfully`);
    } catch (err) {
      alert("Failed to record budget decision.");
    } finally {
  ...
  return (
    ...
    >
      ...
      <div id="pms-new-status-form-content">
        ...
        {budgetData && (
          <FormSection title={budgetData.decision === "Escalated to Director" ? "Escalated Budget Approval" : "Budget Information"}>
            {budgetData.decision === "Escalated to Director" && (
              <Text weight={500} mb="md" color="orange">This application has an escalated budget that requires your approval.</Text>
            )}
            <Grid mb="md">
              <Grid.Col span={12} md={3}>
                <FormField label="Filing Cost:" value={`₹${budgetData.filing_cost}`} />
              </Grid.Col>
              <Grid.Col span={12} md={3}>
                <FormField label="Attorney Fees:" value={`₹${budgetData.attorney_fees}`} />
              </Grid.Col>
              <Grid.Col span={12} md={3}>
                <FormField label="Admin Cost:" value={`₹${budgetData.administrative_cost}`} />
              </Grid.Col>
              <Grid.Col span={12} md={3}>
                <FormField label="Total Cost:" value={`₹${budgetData.total_cost}`} />
              </Grid.Col>
              <Grid.Col span={12} md={6}>
                <FormField label="Status / Decision:" value={budgetData.decision} />
              </Grid.Col>
              <Grid.Col span={12} md={6}>
                <FormField label="Budget Remarks:" value={budgetData.remarks || "No remarks provided"} />
              </Grid.Col>
            </Grid>
            {budgetData.decision === "Escalated to Director" && (
        ...
        <FormSection title="Application Decision">
          <div id="pms-new-status-decision-section">
            <div id="pms-new-status-assign-attorney-section">
              ...
              <Textarea
                placeholder="Provide your feedback (minimum 50 characters for rejection/revision)"
                value={feedback}
