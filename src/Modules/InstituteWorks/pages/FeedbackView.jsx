...
function FeedbackView() {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      await submitFeedback(parseInt(selectedRequestId, 10), rating, comments);
      notifications.show({
        color: "green",
  ...
  const handleReopenRequest = async () => {
    ...
    setIsReopening(true);
    try {
      await reopenRequest(parseInt(selectedReopenRequestId, 10), reopenReason);
      notifications.show({
        color: "green",
    ...
    <Container size="lg" py="xl">
      ...
      <Grid>
                  ...
                  {rating <= 2 && (
                    <Paper p="sm" bg="orange.1" radius="sm" withBorder>
                      <Text size="sm" c="orange.9">
                        ⚠️ Low rating noted. You can reopen this case if issues
                        need further work.
                      </Text>
                    </Paper>
                  )}
        ...
        <Grid.Col span={{ base: 12, md: 6 }}>
          <Paper p="md" radius="md" withBorder>
            ...
            <Stack gap="md">
              <Text size="sm" c="gray">
                Reopen a completed request if issues are found that require
                additional work.
              </Text>
                ...
                <>
                  ...
                  <Paper p="md" bg="red.1" radius="sm" withBorder>
                    <Text size="sm" c="red.9">
                      ⚠️ Reopening will revert the request to work-in-progress
                      state. The engineer will need to re-do the work and
                      resubmit bills.
                    </Text>
                  </Paper>
              ...
              {feedbackHistory.length === 0 ? (
                ...
                feedbackHistory.map((feedback) => (
                  <Table.Tr key={feedback.id}>
                    ...
                    <Table.Td>
                      <Badge
                        color={feedback.reopened ? "orange" : "gray"}
                        variant="light"
                      >
      ...
      <Paper p="md" radius="md" withBorder mt="xl" bg="blue.0">
        ...
        <Stack gap="xs" size="sm">
          <Text>
            <strong>Feedback:</strong> After your request is completed and
            billed, please rate your experience and provide comments to help us
            improve.
          </Text>
          <Text>
            <strong>Reopen:</strong> If you encounter post-repair issues, reopen
            the case immediately so the team can address it.
          </Text>
          <Text>
            <strong>Rating Guide:</strong> 1=Poor, 2=Below Average, 3=Average,
            4=Good, 5=Excellent
          </Text>
        </Stack>
      </Paper>
