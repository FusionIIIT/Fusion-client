...
function CreatedRequestsTable({ requests, isLoading, onRefresh }) {
  const rows = requests.map((request) => (
    <Table.Tr key={request.request_id}>
      ...
      {/* <Table.Td>
        {readField(request, "estimated_budget", "estimatedBudget") != null
      ...
      <Table.Td>
        <Badge
          color={statusBadgeColor(request.iwdAdminApproval)}
          variant="light"
        >
          {request.iwdAdminApproval === 1
            ? "Approved"
            : request.iwdAdminApproval === -1
              ? "Rejected"
              : "Pending"}
      ...
      <Table.Td>
        <Badge
          color={request.engineerProcessed ? "green" : "yellow"}
          variant="light"
        >
          {request.engineerProcessed ? "Created" : "Pending"}
        </Badge>
      </Table.Td>
      ...
      {/* <Table.Td>
        <Group gap="xs" wrap="nowrap">
          <Button
            size="xs"
            variant="light"
            onClick={() => onForward(request.file_id)}
