...
function DeanDirectorQueueTable({ rows, isLoading, onRefresh, onAction }) {
  return (
            ...
            {rows.length > 0 ? (
              rows.map((item) => (
                <Table.Tr key={item.file_id}>
                  ...
                  <Table.Td>
                    {readField(item, "estimated_budget", "estimatedBudget") !=
                    null
                      ? `Rs ${currencyFormatter.format(Number(readField(item, "estimated_budget", "estimatedBudget")))}`
                      : "-"}
                  </Table.Td>
                  <Table.Td>
                    {readField(item, "is_priority", "isPriority")
                      ? "Priority"
                      : "Normal"}
                  </Table.Td>
                  <Table.Td>
                    {readField(item, "next_approver", "nextApprover") || "-"}
                  </Table.Td>
...
DeanDirectorQueueTable.propTypes = {
...
export default DeanDirectorQueueTable;
