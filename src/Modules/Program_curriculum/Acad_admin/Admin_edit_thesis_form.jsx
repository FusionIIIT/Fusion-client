...
import { fetchDisciplinesData, fetchThesisDetails } from "../api/api";
import { host } from "../../../routes/globalRoutes";

function Admin_edit_thesis_form() {
  const form = useForm({
    initialValues: {
      code: "",
      name: "",
    ...
    validate: {
      code: (value) => (!value ? "Thesis code is required" : null),
      name: (value) => (!value ? "Thesis name is required" : null),
      credit: (value) =>
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [disciplinesResponse, thesisData] = await Promise.all([
          fetchDisciplinesData(),
          fetchThesisDetails(id),
        ]);
        ...
        form.setValues({
          code: thesisData.code || "",
          name: thesisData.name || "",
          credit: thesisData.credit || 0,
          discipline: thesisData.discipline?.toString() || "",
          programme_type: thesisData.programme_type || "",
        });
      } catch (fetchError) {
        notifications.show({
          title: "Error",
          message: "Failed to load thesis details. Please try again.",
          color: "red",
          autoClose: 4000,
        });
      } finally {
  ...
  const handleSubmit = async (values) => {
    const apiUrl = `${host}/programme_curriculum/api/admin_update_thesis/${id}/`;
    ...
    try {
        ...
        notifications.show({
          ...
          message: (
            <div>
              <Text size="sm" mb={8}>
                <strong>
                  Thesis "{values.name}" ({values.code}) has been updated.
                </strong>
              </Text>
          ...
          message: (
            <div>
              <Text size="sm" mb={8}>
                <strong>
                  {errorData.error ||
                    "Unable to update thesis. Please try again."}
                </strong>
              </Text>
            </div>
          ),
      ...
      notifications.show({
        ...
        message: (
          <div>
            <Text size="sm" mb={8}>
              <strong>Connection error occurred while updating thesis.</strong>
            </Text>
            <Text size="xs" c="gray.7">
                ...
                >
                  <tbody>
                    ...
                    <tr>
                      ...
                      >
                        <TextInput
                          placeholder="e.g. CS799, EC799, ME799"
                    ...
                    <tr>
                      ...
                      >
                        <TextInput
                          placeholder="e.g. PhD Thesis in Computer Science"
...
export default Admin_edit_thesis_form;
