...
export default function OfferDetailPage() {
  ...
  return (
    ...
    >
      ...
      <Text>
        <strong>Package:</strong> {offer.ctc} LPA
      </Text>
      <Text>
        <strong>Status:</strong>{" "}
        <Badge
          color={
            offer.invitation === "PENDING"
              ? "yellow"
      ...
      <Text mt="md" color={timeLeft > 0 ? "blue" : "red"}>
        <strong>Time left to respond:</strong> {formatTimeLeft(timeLeft)}
      </Text>
      <Group mt="xl" position="right">
        {offer.invitation === "PENDING" && timeLeft > 0 && (
          <>
            <Button color="green" onClick={() => handleAction("ACCEPTED")}>
              Accept
            </Button>
            <Button color="red" onClick={() => handleAction("REJECTED")}>
              Decline
            </Button>
          </>
        )}
