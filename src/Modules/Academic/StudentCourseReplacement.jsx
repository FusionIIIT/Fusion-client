...
export default function StudentCourseReplacement() {
  ...
  const pickCourse = (idx, val) => {
    setSlots(slots.map((s, i) => (i === idx ? { ...s, newCourse: val } : s)));
  };
  ...
  const availableSlots = slots.filter(
    (s) => !existingReqs.some((req) => req.slot === s.name),
  );
  ...
  return (
    <Card>
      {availableSlots.length > 0 ? (
          ...
          {availableSlots.map((s) => {
            ...
            return (
              <SlotCard key={s.id} name={s.name}>
                <SlotRow
                  primary={courseLabel(s.old_course)}
                  secondary="Current course"
                />
                <SlotRow
                  primary="New course"
                  control={
                    <SlotSelect
                      label={`New course for ${s.name}`}
                      placeholder="Select new course…"
                      value={s.newCourse}
                      onChange={(val) => pickCourse(originalIdx, val)}
                      options={s.new_courses.map((c) => ({
                        value: String(c.id),
                        label: `${courseLabel(c)} — ${c.seats_available} seats`,
                      }))}
                    />
                  }
                />
        ...
        <Alert color="gray">
          You have already submitted replacement requests for all available
          slots. Check the "Your Requests" tab to view them.
        </Alert>
      ) : (
        <Alert color="gray">No slots available for course replacement.</Alert>
      )}
      ...
      >
        {toSubmit.length === 0 ? (
          ...
          <>
            {toSubmit.map((s) => {
              const nc = s.new_courses.find(
                (c) => String(c.id) === s.newCourse,
              );
              return (
                <SlotCard key={`${s.id}-${s.newCourse}`} name={s.name}>
                  <SlotRow
                    primary={courseLabel(s.old_course)}
                    secondary="Replacing"
                  />
                  <SlotRow
                    primary={nc ? courseLabel(nc) : "Invalid"}
                    secondary="With"
                  />
                </SlotCard>
              );
