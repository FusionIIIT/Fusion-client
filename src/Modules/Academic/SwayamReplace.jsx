...
import { Alert, Select, Button, Grid, Text, Tabs, Loader } from "@mantine/core";
...
import { FormRow, FormSection } from "../../ui/components/FormSection";
import SlotCard from "./components/SlotCard";
import SlotRow from "./components/SlotRow";
import SwayamYourRequests from "./SwayamYourRequests";

export default function SwayamReplace({
  showOnlyForm = false,
  onSubmitSuccess,
  refreshKey = 0,
}) {
  ...
  const fetchReplaceData = async () => {
    ...
    try {
      ...
      if (data.has_existing) {
        ...
        data.available_semesters &&
        data.available_semesters.length > 0
      ) {
        ...
        setRegistrationError(
          "No OE courses found from semester 3 onwards. Cannot proceed with replacement.",
        );
      }
    } catch (err) {
      const errorMsg =
        err?.response?.data?.error || "Failed to load replacement data.";
  ...
  const isFormComplete = () => {
    const basicComplete =
      selectedSourceCourse && selectedTargetSlot && selectedTargetCourse;
      ...
      if (response.status === 201 || response.status === 200) {
        setSuccessMessage(
          response.data.message ||
            "Swayam replacement request submitted successfully! Awaiting Academic Admin approval.",
        );
  ...
  if (hasPendingRequest) {
    return (
      <Alert color="teal" mb="md">
        You already have a pending or approved replacement request. Please wait
        for the admin to process it.
      </Alert>
    );
  }
  ...
  const formContent = registrationError ? (
    ...
    <>
      ...
      <Text size="sm" c="dimmed" mb="md">
        Replace an existing Swayam course with a new one. This request requires
        Academic approval.
      </Text>
      ...
      {singleSlotAllowed ? (
        <Alert color="orange" mb="md" variant="light">
          ...
          <ul style={{ margin: "6px 0 0 0", paddingLeft: "20px" }}>
            <li>
              If you have done a Swayam course in a previous semester and it was{" "}
              <strong>not used for replacing an Elective</strong> (i.e., it
              remained as Extra Credit), you can select <strong>only 1</strong>{" "}
              new Swayam course.
            </li>
          </ul>
          <div
            style={{ textAlign: "center", fontWeight: "bold", margin: "4px 0" }}
          >
            OR
          </div>
          <ul style={{ margin: "0", paddingLeft: "20px" }}>
            <li>
              If you have{" "}
              <strong>
                not done any Swayam course in your previous semesters
              </strong>
              , then you have to opt <strong>two Swayam Courses</strong>.
            </li>
          </ul>
        </Alert>
      ) : isCurrentSemester ? (
        <Alert color="orange" mb="md" variant="light">
          <strong>Both Slots Required:</strong> The registered course will be
          dropped and two new Swayam courses will be registered upon approval.
        </Alert>
      ) : (
      ...
      <FormSection title="Course to replace" first>
        {existingSwayamCourse ? (
          <SlotCard
            name={existingSwayamCourse.slot}
            meta={`Semester ${existingSwayamCourse.semester}`}
          >
            <SlotRow
              primary={courseLabel(existingSwayamCourse)}
          ...
          <Grid.Col span={{ base: 12, sm: 6 }}>
            <Select
              ...
              data={
                existingSwayamCourse
                  ? targetSlots.filter(
                      (slot) => slot.value !== selectedTargetSlot2,
                    )
                  : targetSlotsForSelection
                      .filter(
                        (slot) =>
                          slot.id !== undefined &&
                          slot.id !== null &&
                          String(slot.id) !== selectedTargetSlot2,
                      )
                      .map((slot) => ({
                        value: String(slot.id),
                        label: slot.name,
              ...
              data={targetCourses
                .filter(
                  (course) =>
                    course.id !== undefined &&
                    course.id !== null &&
                    String(course.id) !== selectedTargetCourse2,
                )
                .map((course) => ({
                  value: String(course.id),
                  label: courseLabel(course),
          ...
          <Grid.Col span={{ base: 12, sm: 6 }}>
            <Select
              ...
              data={
                existingSwayamCourse
                  ? targetSlots.filter(
                      (slot) => slot.value !== selectedTargetSlot,
                    )
                  : targetSlotsForSelection2
                      .filter(
                        (slot) =>
                          slot.id !== undefined &&
                          slot.id !== null &&
                          String(slot.id) !== selectedTargetSlot,
                      )
                      .map((slot) => ({
                        value: String(slot.id),
                        label: slot.name,
              ...
              data={targetCourses2
                .filter(
                  (course) =>
                    course.id !== undefined &&
                    course.id !== null &&
                    String(course.id) !== selectedTargetCourse,
                )
                .map((course) => ({
                  value: String(course.id),
                  label: courseLabel(course),
