...
import { hodThesisChangeRequestReviewRoute } from "../../routes/academicRoutes";

export default function HODThesisChangeRequestModal({
  request,
  onClose,
  refresh,
}) {
  ...
  const handle = async (approve) => {
    ...
    try {
      await axios.post(
        hodThesisChangeRequestReviewRoute(request.id),
        { approve, remarks },
        {
          headers: {
      ...
      showNotification({
        ...
        message: approve
          ...
          : "Sent back to supervisor/student.",
  ...
  return (
    <Modal
      opened
      onClose={onClose}
      title="Convener (DPGC) Review — Thesis Change Request"
      size="80%"
    >
      <Stack gap="md">
        ...
        <Text fw={500}>Requested Change (all consents already given)</Text>
        <Table striped highlightOnHover>
          <Table.Tbody>
...
HODThesisChangeRequestModal.propTypes = {
