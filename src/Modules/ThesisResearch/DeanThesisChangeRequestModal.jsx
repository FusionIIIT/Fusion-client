...
import { deanThesisChangeRequestReviewRoute } from "../../routes/academicRoutes";

export default function DeanThesisChangeRequestModal({
  request,
  onClose,
  refresh,
}) {
  ...
  const handle = async (approve) => {
    ...
    try {
      await axios.post(
        deanThesisChangeRequestReviewRoute(request.id),
        { approve, remarks },
        {
          headers: {
      ...
      showNotification({
        ...
        message: approve
          ? "Change applied to the thesis topic."
          : "Request rejected; the HOD has been notified.",
  ...
  return (
    <Modal
      opened
      onClose={onClose}
      title="Dean Academic — Thesis Change Final Approval"
      size="80%"
    >
      <Stack gap="md">
        ...
        <Text fw={500}>
          Requested Change (all consents + HOD approval already given)
        </Text>
        <Table striped highlightOnHover>
          <Table.Tbody>
...
DeanThesisChangeRequestModal.propTypes = {
